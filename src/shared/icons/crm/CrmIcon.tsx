...
import { normalizeCrmIconName } from './crmIcon.utils';
import type { CrmIconName } from './crmIcon.types';

interface CrmIconProps extends Omit<React.SVGProps<SVGSVGElement>, 'name'> {
  name: CrmIconName | string | null | undefined;
  title?: string;
}
 ...
 */
const CRM_ICON_GLYPHS: Record<CrmIconName, React.ReactNode> = {
  database: (
    <>
...
export const CrmIcon: React.FC<CrmIconProps> = ({
  ...
  const normalizedName = normalizeCrmIconName(name);
  ...
  return (
    ...
    >
      ...
      {CRM_ICON_GLYPHS[normalizedName]}
    </svg>
  );
};
