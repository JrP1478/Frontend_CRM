...
Validación Source -> KPI para CLIENTE_A V1
SOLO LECTURA.

Ejecutar en:
    legacy_crm_reporteria
...
- Evitar mezclar CLIENTE_A GOBIERNO con el BI CLIENTE_A Administrativo.
*/

USE [legacy_crm_reporteria];
GO
SET NOCOUNT ON;
...
DECLARE @CampLegacyCrm VARCHAR(15) =
...
SELECT
    ...
    @CampLegacyCrm AS campaign_source_code,
    @StartDate AS campaign_start_date;


PRINT '============================================================';
PRINT '1. UNIVERSO DE CARTERAS CLIENTE_A ADMINISTRATIVO';
PRINT '============================================================';

IF OBJECT_ID('tempdb..#ClienteAPortfolioScope') IS NOT NULL
    DROP TABLE #ClienteAPortfolioScope;

SELECT DISTINCT
    p.nId_Cartera,
    LTRIM(RTRIM(p.cartera)) AS cartera
INTO #ClienteAPortfolioScope
FROM dbo.PBI_CLIENTE_A_CORP_ADMINISTRATIVO AS p
WHERE p.AñoLegacyCrm = @CampaignYear
  AND p.CampLegacyCrm = @CampLegacyCrm;

CREATE UNIQUE CLUSTERED INDEX IX_ClienteAPortfolioScope
    ON #ClienteAPortfolioScope(nId_Cartera);

SELECT
    COUNT(*) AS portfolios_in_scope
FROM #ClienteAPortfolioScope;
...
WHERE g.cCli_Nombre = 'CLIENTE_A CORPORATIVO'
  ...
  (
      SELECT 1
      FROM #ClienteAPortfolioScope AS s
...
PRINT '============================================================';
PRINT '9. FRESCURA CLIENTE_A ADMINISTRATIVO';
PRINT '============================================================';
