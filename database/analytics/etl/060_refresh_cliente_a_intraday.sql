/*
Portfolio Control Center - orquestador intradia CLIENTE_A

- Consume solo corridas CORTO/CICLO/HECHO de legacy_crm_reporteria.
- Si id_corrida no cambio, termina sin ejecutar LIVE/ADVISOR.
- Solo avanza CLIENTE_A_INTRADAY_UPSTREAM cuando ambos ETL terminan bien.
...
CREATE OR ALTER PROCEDURE etl.usp_refresh_cliente_a_intraday
...
BEGIN
    ...
    DECLARE
        ...
        @lock_resource NVARCHAR(255) = CONCAT(
            N'analytics_cliente_a_intraday_',
            @crm_client_id
        ),
    ...
    /*
       ...
       legacy_crm_reporteria impide leer rpt_gestiones_pagos_final mientras un
    ...
    */
    EXEC @upstream_lock_result = legacy_crm_reporteria.sys.sp_getapplock
    ...
    BEGIN TRY
        ...
        FROM legacy_crm_reporteria.dbo.pbi_ciclo_ejecucion AS e
        WHERE e.ciclo = 'CORTO'
          AND e.fase = 'CICLO'
          AND e.estado = 'HECHO'
        ...
        WHERE w.source_code = 'CLIENTE_A_INTRADAY_UPSTREAM';
        ...
        BEGIN
            EXEC legacy_crm_reporteria.sys.sp_releaseapplock
                @Resource = N'pbi_ciclo',
                @LockOwner = 'Session';
        ...
        EXEC etl.usp_load_cliente_a_live_operations
            @crm_client_id = @crm_client_id;

        EXEC etl.usp_load_cliente_a_advisor_daily
            @crm_client_id = @crm_client_id;
        ...
        SELECT
            ...
            @advisor_source_as_of = MAX(
                CASE
                    WHEN w.source_code = 'CLIENTE_A_ADVISOR_DAILY'
                        THEN w.last_source_datetime
                END
            )
        FROM etl.watermark AS w
