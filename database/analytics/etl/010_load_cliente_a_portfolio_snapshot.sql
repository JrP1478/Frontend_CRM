...
ETL CLIENTE_A: dimensiones + snapshot de cartera

FUENTE:
    legacy_crm_reporteria.dbo.PBI_CLIENTE_A_CORP_ADMINISTRATIVO
...
- CLIENTE_A está encapsulado en este adaptador de fuente, nunca en React;
...
CREATE OR ALTER PROCEDURE etl.usp_load_cliente_a_portfolio_snapshot
    @crm_client_id    INT,
...
BEGIN
    ...
    FROM legacy_crm_reporteria.dbo.PBI_CLIENTE_A_CORP_ADMINISTRATIVO AS s
    WHERE s.AñoLegacyCrm = @campaign_year
      AND s.CampLegacyCrm = @source_campaign_code;

    IF ISNULL(@source_rows, 0) = 0
        THROW 51105, 'La fuente CLIENTE_A no tiene filas para la campaña solicitada.', 1;
