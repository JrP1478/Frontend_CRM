...
`scripts/analytics/load_legacy_crm_usuario_snapshot.py`

El loader realiza en una sola ejecución:

1. lectura de `PRIVATE_HOST / legacy_crm_cob.dbo.av_Usuario`;
2. reemplazo de `staging.legacy_crm_usuario_current` para `LEGACY_CRM_COB_45`;
3. ejecución de `etl.usp_load_cliente_a_supervisor_hierarchy`;
4. validación de staging, watermark y mapping;
5. commit único en `legacy_crm_analytics`.

## Restricción de conectividad

El runtime elegido debe poder abrir ambas conexiones ODBC:

- `PRIVATE_HOST:51601 / legacy_crm_cob`;
- `PRIVATE_HOST:51601 / legacy_crm_analytics`.
...
`export_legacy_crm_usuario_snapshot_sql.py` se conserva como mecanismo de contingencia
