...
SELECT
    OBJECT_SCHEMA_NAME(t.object_id, DB_ID('legacy_crm_reporteria'))
        AS trigger_schema,
    t.name AS trigger_name,
    OBJECT_NAME(t.parent_id, DB_ID('legacy_crm_reporteria'))
        AS parent_table,
    t.is_disabled,
    m.definition
FROM legacy_crm_reporteria.sys.triggers AS t
LEFT JOIN legacy_crm_reporteria.sys.sql_modules AS m
    ON m.object_id = t.object_id
WHERE t.parent_id IN
(
    OBJECT_ID('legacy_crm_reporteria.dbo.RPTC_253_PRODUCCIONGENERALPORGESTOR_ACUMULADO'),
    OBJECT_ID('legacy_crm_reporteria.dbo.RPTC_253_PRODUCCIONGENERALPORGESTOR_DIA')
)
...
SELECT
    ...
    sy.base_object_name
FROM legacy_crm_reporteria.sys.synonyms AS sy
