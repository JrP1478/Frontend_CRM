...
Soporte para meta mensual CLIENTE_A desde base-goals.xlsx
Motor: SQL Server
...
Este script:
- crea staging normalizado para la meta mensual CLIENTE_A;
...
IF OBJECT_ID('staging.cliente_a_goal_monthly', 'U') IS NULL
BEGIN
    CREATE TABLE staging.cliente_a_goal_monthly
    (
        ...
        loaded_at                   DATETIME2(3) NOT NULL
            CONSTRAINT DF_staging_cliente_a_goal_monthly_loaded_at
            DEFAULT (SYSUTCDATETIME()),

        CONSTRAINT PK_staging_cliente_a_goal_monthly
            PRIMARY KEY (source_code, campaign_code),

        CONSTRAINT CK_staging_cliente_a_goal_monthly_month
            CHECK (campaign_month BETWEEN 1 AND 12),

        CONSTRAINT CK_staging_cliente_a_goal_monthly_amount
            CHECK (target_recovered_amount >= 0),

        CONSTRAINT CK_staging_cliente_a_goal_monthly_rows
            CHECK (source_rows > 0)
    );
END;
GO
...
CREATE OR ALTER PROCEDURE etl.usp_load_cliente_a_target_monthly
    @crm_client_id   INT,
    @campaign_code   VARCHAR(7),
    @source_code     VARCHAR(50) = 'CLIENTE_A_BASE_GOALS'
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
    ...
    FROM staging.cliente_a_goal_monthly AS s
    ...
    BEGIN TRY
        ...
        WHERE source_code = 'CLIENTE_A_TARGET_MONTHLY';
