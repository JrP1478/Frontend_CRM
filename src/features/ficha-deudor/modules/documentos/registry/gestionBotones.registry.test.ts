...
export const suite = defineSuite('gestionBotones.registry', [
  test('reconoce los nombres técnicos actualmente implementados', () => {
    ...
    assert.equal(isGestionBotonImplemented('adicionalClienteB'), true);
    assert.equal(isGestionBotonImplemented('ADICIONAL CLIENTE_B'), true);
    assert.equal(isGestionBotonImplemented('adicional_cliente_b'), true);
  }),
