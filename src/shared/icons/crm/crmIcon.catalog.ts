...
export const CRM_ICON_CATALOG: readonly CrmIconDefinition[] = [
...
export const CRM_ICON_BY_NAME = new Map<CrmIconName, CrmIconDefinition>(
  CRM_ICON_CATALOG.map((icon) => [icon.name, icon])
);

export const CRM_ICON_CATALOG_BY_CATEGORY = CRM_ICON_CATALOG.reduce(
  (catalog, icon) => {
  ...
  },
  {
    ...
    seguridad: [],
    generales: [],
  } as Record<CrmIconCategory, CrmIconDefinition[]>
);
