...
Diagnóstico de fuente CLIENTE_A EVOL antes de implementar su ETL
Motor: SQL Server 2017+

Ejecutar DENTRO de legacy_crm_analytics.
...
DECLARE @source_object SYSNAME =
    N'legacy_crm_reporteria.dbo.PBI_CARTERA_DIA_CLIENTE_A_CORP_ADMINISTRATIVO_EVOL';

DECLARE @source_object_id INT = OBJECT_ID(@source_object);

IF @source_object_id IS NULL
    THROW 51900, 'No existe la fuente CLIENTE_A EVOL esperada.', 1;
...
INNER JOIN legacy_crm_reporteria.sys.types AS t
...
SELECT
    ...
    CASE
        ...
        (
            ...
            'DIALEGACY_CRM', 'DIACAMPANA', 'DIACAMPANIA'
        ) THEN 2
...
BEGIN
    ...
    BEGIN
        SET @candidate_sql = N'
            ...
            FROM legacy_crm_reporteria.dbo.PBI_CARTERA_DIA_CLIENTE_A_CORP_ADMINISTRATIVO_EVOL AS s
            WHERE s.AñoLegacyCrm = @campaign_year
              AND s.CampLegacyCrm = @source_campaign_code;';
    END
    ELSE
    BEGIN
        SET @candidate_sql = N'
