...
ETL CLIENTE_A - dimensión y producción diaria por asesor

FUENTES:
- legacy_crm_reporteria.dbo.rpt_gestiones_pagos_final
- legacy_crm_reporteria.dbo.rpt_ref_usuario
- legacy_crm_reporteria.dbo.RPTC_253_PRODUCCIONGENERALPORGESTOR_ACUMULADO
...
- advisor_document = DNI único dentro del scope CLIENTE_A/cartera/mes
...
CREATE OR ALTER PROCEDURE etl.usp_load_cliente_a_advisor_daily
    @crm_client_id       INT,
    @as_of_at            DATETIME2(3) = NULL,
    @campaign_year       SMALLINT = NULL,
    @campaign_month      TINYINT = NULL,
    @source_client_name  VARCHAR(150) = 'CLIENTE_A CORPORATIVO'
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
    ...
    INTO #Source
    FROM legacy_crm_reporteria.dbo.rpt_gestiones_pagos_final AS t
    ...
    IF ISNULL(@source_rows, 0) = 0
        THROW 51804, 'No hay gestiones CLIENTE_A en el scope solicitado.', 1;
    ...
    /* ============================================================
       4. DNI dentro del MISMO scope CLIENTE_A
    ...
    (
        ...
        FROM legacy_crm_reporteria.dbo.RPTC_253_PRODUCCIONGENERALPORGESTOR_ACUMULADO AS r
        INNER JOIN
        (
    ...
    (
        ...
        THROW 51807, 'Conflicto de DNI por nId_Usuario dentro del scope CLIENTE_A.', 1;
    ...
    BEGIN TRY
        ...
        /*
          ...
          enteramente a este adaptador para CLIENTE_A V1. Reemplazar MTD evita
        ...
        */
        ...
        WHERE source_code = 'CLIENTE_A_ADVISOR_DAILY';
