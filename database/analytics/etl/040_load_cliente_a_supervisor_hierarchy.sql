...
ETL CLIENTE_A - jerarquía Supervisor -> Asesor

IMPORTANTE:
Este procedimiento SE EJECUTA en legacy_crm_analytics (PRIVATE_HOST),
pero YA NO consulta legacy_crm_cob directamente.
...
CREATE OR ALTER PROCEDURE etl.usp_load_cliente_a_supervisor_hierarchy
    @crm_client_id  INT,
    @source_code    VARCHAR(50) = 'LEGACY_CRM_COB_45',
...
BEGIN
    ...
    LEFT JOIN staging.legacy_crm_usuario_current AS u
        ON u.source_code = @source_code
       AND u.nId_Usuario = TRY_CONVERT(INT, a.source_advisor_id)

    LEFT JOIN staging.legacy_crm_usuario_current AS su
    ...
    BEGIN TRY
        ...
        */
        ...
        WHERE source_code = 'CLIENTE_A_SUPERVISOR_HIERARCHY';
