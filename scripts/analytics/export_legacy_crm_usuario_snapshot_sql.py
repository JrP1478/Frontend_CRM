...
Exporta legacy_crm_cob.dbo.av_Usuario a un SQL de staging para legacy_crm_analytics.

Este script SOLO se conecta al origen accesible:
    PRIVATE_HOST\MSSQLSERVER,51601 / legacy_crm_cob

NO se conecta a legacy_crm_analytics.
...
Uso:
    python scripts/analytics/export_legacy_crm_usuario_snapshot_sql.py

Opcional:
    python scripts/analytics/export_legacy_crm_usuario_snapshot_sql.py \
        --output /tmp/legacy_crm_usuario_snapshot.sql
"""
...
def build_sql(source_as_of_at: datetime, rows) -> str:
    ...
    lines.extend(
        [
            "/*",
            "Portfolio Control Center - ETAPA 6 / Avance 3",
            "Snapshot generado desde PRIVATE_HOST / legacy_crm_cob.dbo.av_Usuario",
            f"source_as_of_at: {source_as_of_at}",
            f"rows: {len(rows)}",
            "",
            "EJECUTAR EN SSMS CONTRA:",
            r"PRIVATE_HOST\MSSQLSERVER,51601 / legacy_crm_analytics",
            "*/",
            "",
            ...
            "    THROW 51970, "
            "'No existe staging.legacy_crm_usuario_current. Ejecutar primero 007.', 1;",
            "GO",
            "",
            "BEGIN TRY",
            "    BEGIN TRANSACTION;",
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default="/tmp/legacy_crm_usuario_snapshot_LEGACY_CRM_COB_45.sql",
        help="Archivo SQL de salida.",
    )
    args = parser.parse_args()

    connection_string = require_env("LEGACY_CRM_COB_CONNECTION_STRING")
    ...
    print(f"Fuente: legacy_crm_cob.dbo.av_Usuario")
