...
Carga staging de legacy_crm_cob.av_Usuario -> legacy_crm_analytics y ejecuta la jerarquía.
...
Preflight sin escrituras:
    python scripts/analytics/load_legacy_crm_usuario_snapshot.py --check-only

Ejecución:
    python scripts/analytics/load_legacy_crm_usuario_snapshot.py
"""
...
SOURCE_CODE = "LEGACY_CRM_COB_45"
SOURCE_DATABASE = "legacy_crm_cob"
TARGET_DATABASE = "legacy_crm_analytics"
HIERARCHY_WATERMARK = "CLIENTE_A_SUPERVISOR_HIERARCHY"
...
TARGET_INSERT_SQL = """
INSERT INTO staging.legacy_crm_usuario_current
(
    source_code,
    source_as_of_at,
    nId_Usuario,
...
def fetch_source_rows(
    ...
    with pyodbc.connect(connection_string, autocommit=True) as connection:
        ...
        if not rows:
            raise RuntimeError(
                "legacy_crm_cob.dbo.av_Usuario no devolvio registros."
            )
    ...
    with pyodbc.connect(connection_string, autocommit=True) as connection:
        ...
        row = cursor.execute(
            ...
            SELECT
                ...
                CASE
                    WHEN OBJECT_ID(
                        'staging.legacy_crm_usuario_current',
                        'U'
        ...
        if row.has_staging != 1:
            raise RuntimeError(
                "No existe staging.legacy_crm_usuario_current en legacy_crm_analytics."
            )
        if row.has_hierarchy_proc != 1:
            raise RuntimeError(
                "No existe etl.usp_load_cliente_a_supervisor_hierarchy."
            )
        ...
        current_staging = cursor.execute(
            ...
            FROM staging.legacy_crm_usuario_current
            WHERE source_code = ?;
            """,
            SOURCE_CODE,
        ).fetchone()
...
def validate_loaded_transaction(
    ...
    if (
        ...
        raise RuntimeError(
            "staging.legacy_crm_usuario_current mezcla timestamps de snapshots."
        )
    ...
    mapping_differences = cursor.execute(
        ...
        INNER JOIN staging.legacy_crm_usuario_current AS u
            ON u.source_code = ?
...
def load_target(
    ...
    try:
        ...
        cursor.execute(
            """
            DELETE FROM staging.legacy_crm_usuario_current
            WHERE source_code = ?;
            """,
            SOURCE_CODE,
        )
        ...
        cursor.execute(
            """
            EXEC etl.usp_load_cliente_a_supervisor_hierarchy
                @crm_client_id = ?,
                @source_code = ?,
                @as_of_at = ?,
                @effective_date = ?;
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=95,
        help="crm_client_id de CLIENTE_A en analytics.dim_client (default: 95)",
    )
    parser.add_argument(
        "--check-only",
        action="store_true",
