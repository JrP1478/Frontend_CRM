...
CLIENTE_A en legacy_crm_analytics.

El script NO se conecta a legacy_crm_analytics. Esto permite usarlo desde el Ubuntu
actual aunque ese host siga sin conectividad ODBC hacia PRIVATE_HOST.
...
Salida por defecto:
    /tmp/cliente_a_goal_snapshot_2026-08.sql
...
- debe existir una única fila CLIENTE_A con META_PAGOS válida en el snapshot del Excel
...
SOURCE_CODE = "CLIENTE_A_BASE_GOALS"
...
def build_snapshot(
    ...
    for row in rows:
        if normalize_text(row.get("CARTERA")) != "CLIENTE_A":
            continue
        ...
        if amount < 0:
            raise ValueError("META_PAGOS no puede ser negativa para CLIENTE_A.")

        matched.append((row, amount))

    if not matched:
        raise ValueError(
            "No se encontró una fila CARTERA=CLIENTE_A con META_PAGOS válida "
            "en el snapshot actual de base-goals.xlsx."
        )

    if len(matched) != 1:
        raise ValueError(
            "Se encontraron múltiples filas CARTERA=CLIENTE_A con META_PAGOS válida. "
...
def build_sql(snapshot: GoalSnapshot, crm_client_id: int) -> str:
    ...
    return f"""/*
Portfolio Control Center - ETAPA 6 / Avance 4
Meta mensual CLIENTE_A exportada desde base-goals.xlsx
...
EJECUTAR EN SSMS CONTRA legacy_crm_analytics DESPUÉS DE 011_portfolio_v1_cliente_a_target_support.sql
*/

USE legacy_crm_analytics;
GO
...
BEGIN TRY
    ...
    DELETE FROM staging.cliente_a_goal_monthly
    WHERE source_code = '{SOURCE_CODE}'
      AND campaign_code = '{snapshot.campaign_code}';

    INSERT INTO staging.cliente_a_goal_monthly
    (
        source_code,
        campaign_code,
        campaign_year,
    ...
    EXEC etl.usp_load_cliente_a_target_monthly
    ...
    SELECT
        ...
        'CLIENTE_A_TARGET_IMPORT_OK' AS assessment;
END TRY
BEGIN CATCH
    IF @@TRANCOUNT > 0
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=95,
        help="crm_client_id de CLIENTE_A en Analytics. Default: 95.",
    )
    parser.add_argument(
        "--output",
        default=None,
        help="Archivo SQL de salida. Default: /tmp/cliente_a_goal_snapshot_<campaign>.sql",
    )
    ...
    output = (
        ...
        if args.output
        else Path(f"/tmp/cliente_a_goal_snapshot_{campaign_code}.sql")
    )
    ...
    print("")
    print("Ejecuta ese SQL en SSMS contra legacy_crm_analytics.")
    return 0
