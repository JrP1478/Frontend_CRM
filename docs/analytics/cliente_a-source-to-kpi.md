# Portfolio Control Center - CLIENTE_A Source to KPI

## Estado del análisis

Este documento actualiza el mapeo Source -> KPI del Portfolio Control Center para CLIENTE_A a partir del proyecto Power BI (`.pbip`) de `BI_cliente_a` y del análisis previo de `GESTION-COB2`.

La extracción del PBIP permite confirmar fórmulas DAX, transformaciones Power Query/M, relaciones del modelo y orígenes físicos de las principales tablas lógicas de CLIENTE_A. Aún no se diseña el Analytics DB: primero se debe inspeccionar la definición SQL y el grain de los objetos físicos identificados.

## Fuentes físicas confirmadas de BI_cliente_a

### SQL Server / base `legacy_crm_reporteria`

| Tabla semántica | Objeto físico utilizado por Power Query |
|---|---|
| `dbEffectiveness` | `dbo.PBI_CLIENTE_A_CORP_ADMINISTRATIVO` |
| `dbIntensity` | `dbo.PBI_CLIENTE_A_CORP_ADMINISTRATIVO` |
| `dbEvolutionary` | `dbo.PBI_CARTERA_DIA_CLIENTE_A_CORP_ADMINISTRATIVO_EVOL` |
| `dbProduction` | `dbo.PBI_CARTERA_DIA_CLIENTE_A_CORP_ADMINISTRATIVO_PROD` |

El PBIP conecta estos objetos mediante `Sql.Database(..., "legacy_crm_reporteria")`. Para la arquitectura Analytics no se debe copiar la dirección del servidor del PBIX al código: la conexión será configuración segura de infraestructura.
...
| `dbUsers` | Excel `base-users-cliente_a.xlsx`, tabla `users.cliente_a.corp`, almacenado actualmente en SharePoint |
...
## Definiciones confirmadas por el modelo actual de CLIENTE_A
...
**Definición canónica CLIENTE_A V1:** cantidad de clientes/deudores únicos pertenecientes a la asignación seleccionada.
...
Por tanto, la definición que se había dejado provisional queda confirmada para CLIENTE_A:
...
Se debe usar la versión normalizada a soles para el Portfolio CLIENTE_A salvo que en Analytics se defina soporte multimoneda explícito.
...
**Importante:** en `BI_cliente_a`, el denominador de contactabilidad es la cartera asignada (`QTY CLIENTES`), no la cartera gestionada. Analytics debe preservar inicialmente esta definición para no producir cifras distintas al BI oficial.
...
Esto proporciona la regla de CLIENTE_A para el contacto directo/titular que utilizaremos como candidato de RPC. Antes de unificar clientes, se debe contrastar con la clasificación `CD/CI/NC/CT` de `GESTION-COB2`.
...
La fórmula DAX oficial del BI CLIENTE_A queda confirmada:
...
S/. PAGOS = SUM(PAGOS_SOLES_CLIENTE_A)
```
...
`dbProduction` proviene de `dbo.PBI_CARTERA_DIA_CLIENTE_A_CORP_ADMINISTRATIVO_PROD` y expone:
...
`dbIntensity` reutiliza `dbo.PBI_CLIENTE_A_CORP_ADMINISTRATIVO`, selecciona los contadores por canal:
...
1. conservar `AVG GESTIÓN MASIVA` como KPI específico de canales masivos CLIENTE_A; o
...
```text
dbo.PBI_CARTERA_DIA_CLIENTE_A_CORP_ADMINISTRATIVO_EVOL
```
...
## Campaña CLIENTE_A confirmada como mes operativo

El Power Query deriva las etiquetas de campaña desde `CampLegacyCrm`, cuyo sufijo `01..12` se transforma a `ENERO..DICIEMBRE`.

También construye:

```text
DATE ORDER = AñoLegacyCrm + mes(CampLegacyCrm)
NRO ASIGNACION = "[ASIGNACIÓN MM]"
```

Esto respalda la regla funcional acordada:

> Para CLIENTE_A, la campaña del Portfolio Control Center se representa por el mes operativo (año-mes).
...
```text
GOAL PAGOS S/. = SUM(dbGoals[META_PAGOS]) filtrando CARTERA = "CLIENTE_A"
...
`AÑO` y `ASIGNACIÓN` quedan como trazabilidad de la fuente. Si el archivo incorpora múltiples filas vigentes para CLIENTE_A en el futuro, debe validarse una nueva regla de selección antes de automatizarla.
...
`dbUsers` proviene actualmente de `base-users-cliente_a.xlsx` y contiene únicamente:
...
|---|---|---|---|
| Cartera asignada | `dbo.PBI_CLIENTE_A_CORP_ADMINISTRATIVO` | `SUM(CLIENTE UNICO)`, con `Deudor_unico = 1` | Confirmado en PBIP |
...
| Intensidad CLIENTE_A actual | mismo objeto + `dbIntensity` | promedio de contadores por canal | Confirmado; definición canónica pendiente |
| Contactabilidad cartera | mismo objeto | contactados / asignados | Confirmado |
| RPC/contacto directo | mismo objeto | indicador `CD` sobre cliente único | Confirmado para CLIENTE_A; normalización pendiente |
...
| Recaudo | mismo objeto | `SUM(PAGOS_SOLES_CLIENTE_A)` | Confirmado |
...
| Campaña CLIENTE_A | `CampLegacyCrm`, `AñoLegacyCrm` | año-mes operativo | Confirmado |
| Asesor | PROD + `base-users-cliente_a.xlsx` | `DNI_ASESOR -> DNI` | Confirmado |
...
1. `BI_cliente_a` ya consume objetos de reportería dedicados. Debemos inspeccionar su SQL antes de volver a consultar las tablas transaccionales desde cero.
...
Ya no se necesita pedir la estructura completa de la BD. El siguiente paso es inspeccionar solamente estos objetos SQL de `legacy_crm_reporteria`:

```text
dbo.PBI_CLIENTE_A_CORP_ADMINISTRATIVO
dbo.PBI_CARTERA_DIA_CLIENTE_A_CORP_ADMINISTRATIVO_EVOL
dbo.PBI_CARTERA_DIA_CLIENTE_A_CORP_ADMINISTRATIVO_PROD
```
